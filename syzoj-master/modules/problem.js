...
app.post('/problem/:id/submit/:sid', app.multer.fields([{ name: 'answer', maxCount: 1 }]), async (req, res) => {
  let judge_state;
  try {
    let id = parseInt(req.params.id);
    let problem = await Problem.findById(id);
    ...
    if (!problem) throw new ErrorMessage('无此题目。');
    if (!syzoj.config.enabled_languages.includes(req.body.language))){
